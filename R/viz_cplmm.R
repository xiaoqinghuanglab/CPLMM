#' Plot CPLMM trajectories (status-change, normal-only, abnormal-only)
#'
...
plot_cplmm <- function(
  ...
  if (!nrow(trends)) {
    stop("plot_cplmm(): no trends could be computed — check inputs/columns.", call. = FALSE)
  }
  ...
  if (isTRUE(export)) {
    ...
    for (fmt in export_formats) {
      outfile <- file.path(export_dir, paste0(protein, "_cplmm.", fmt))
